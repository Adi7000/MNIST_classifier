...
class ConvNet(nn.Module):
    def __init__(self, hidden_size, hidden_size2, num_classes):
        ...
        self.conv1 = nn.Conv2d(1, conv_outputs, 3)
        self.pool = nn.MaxPool2d(2, 2)
        self.fc1 = nn.Linear(conv_outputs * 26 * 26, hidden_size)
        self.fc2 = nn.Linear(hidden_size, hidden_size2)
        self.fc3 = nn.Linear(hidden_size2, num_classes)

    def forward(self, x):
        # -> n, 1, 28, 28
        x = F.relu(self.conv1(x))             # -> n, 6 (conv_out), 26, 26
        x = x.view(-1, conv_outputs * 26 * 26)
        x = F.relu(self.fc1(x))               
        x = F.relu(self.fc2(x))               
        x = self.fc3(x)                       
        return x

model = ConvNet(hidden_size, hidden_size2, num_classes).to(device)
